    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == RECYCLER_ADAPTER_ITEM) {
            ...
            if (getHeaderLayoutId() != -1) {
                View headerView = LayoutInflater.from(mContext).inflate(getHeaderLayoutId(), parent, false);

                return getHeaderHolder(headerView);
            }
        } else if (viewType == RECYCLER_ADAPTER_FOOTER) {
            if (getFooterLayoutId() != -1) {
                View footerView = LayoutInflater.from(mContext).inflate(getFooterLayoutId(), parent, false);
